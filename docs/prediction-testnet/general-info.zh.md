## API 基本信息
* 本篇列出接口的baseurl: **https://papi.asterdex-testnet.com**
* 所有接口的响应都是 JSON 格式。
* 所有时间、时间戳均为UNIX时间，单位为**毫秒**。

## API Agent Address 设置
* 很多接口需要API Key才可以访问.
* 设置API Key的同时，为了安全，建议设置IP访问白名单.
* **永远不要把你的API Agent Address/Private Key告诉给任何人**

<aside class="warning">
如果不小心泄露了API Agent Address，请立刻删除此API Agent Address
</aside>
...
host = 'https://papi.asterdex-testnet.com'
...
```javascript
{
  "predictionEvents": [
    {
      "eventName": "Who will win the 2026 FIFA World Cup?",  // 预测事件名称
      "eventId": "EVENT4",                                    // 事件标识符
      "oracleSymbol": "",                                     // 预测标的（非价格预测事件为空）
      "predictionType": "MARKETONLY_MULTIPREDICT",              // 预测类型（MARKETONLY_MULTIPREDICT 仅白名单做市商可用）
      "sides": [
        "YES",
        "NO"
      ],                                                      // 预测方向
      "period": null,                                         // 预测周期（分钟）
      "startTime": 1780840786140,                             // 事件开始时间（Unix 毫秒）
      "endTime": 0,                                           // 事件结束时间（0 = 未设置结束时间）
      "status": "LIVE",                                       // 事件状态
      "currentRoundId": null,                                 // 当前轮次ID
      "settleIntervalSeconds": 0,                             // TWAP 价格采样间隔（秒）
      "settlementDataPointCount": 0,                          // TWAP 价格数据点数量
      "reduceOnly": false,                                    // 是否只允许减仓订单
      "category": "FIFA2026",                                 // 事件分类
      "orderTypes": [
        "LIMIT",
        "MARKET"
      ],                                                      // 支持的订单类型
      ...
      "liveRound": null,                                      // 当前进行中的轮次
      "rounds": null,                                         // 历史轮次
      "markets": [                                            // 各结果的预测市场列表
        {
          "marketName": "EVENT4_ALGERIA_WIN",                 // 预测市场名称
          "symbols": [
            "EVENT4_ALGERIA_WIN_YUSDT",
            "EVENT4_ALGERIA_WIN_NUSDT"
          ],                                                  // 该市场的两个交易对；Y = YES，N = NO
          "assets": [
            "EVENT4_ALGERIA_WIN_Y",
            "EVENT4_ALGERIA_WIN_N"
          ],                                                  // 该市场的两个资产；Y = YES，N = NO
          "roundId": 0,                                       // 轮次ID
          "tradingStartTime": 1780840786140,                  // 交易开始时间（Unix 毫秒）
          "tradingEndTime": 0,                                // 交易结束时间（0 = 未设置结束时间）
          "windowStartTime": 1780840786140,                   // 窗口开始时间（Unix 毫秒）
          "windowEndTime": 0,                                 // 窗口结束时间（0 = 未设置结束时间）
          "roundStatus": "TRADING",                           // 市场状态
          "openPrice": null,                                  // 开盘价（结算前为 null）
          "endPrice": null,                                   // 结算价（结算前为 null）
          "winAsset": "EVENT4_ALGERIA_WIN_N",                 // 获胜资产：Y 代表 YES 胜，N 代表 NO 胜
          "settleTime": 0,                                    // 结算时间（0 = 未结算）
          "settleHash": null                                  // 链上结算哈希（未结算时为 null）
        }
      ]
    },
    {
      "eventName": "USD1_UP_DOWN_5M",                         // 预测事件名称
      "oracleSymbol": "USD1USDT",                             // 预测标的
      "predictionType": "UP_DOWN",                           // 预测类型
      "sides": [
        "UP",
        "DOWN"
      ],                                                     // 预测方向
      "period": "10080M",                                    // 预测周期（分钟）
      "startTime": 1778476080000,                            // 事件开始时间（Unix 毫秒）
      "endTime": 1780376880000,                              // 事件结束时间（Unix 毫秒）
      "status": "LIVE",                                      // 事件状态
      "currentRoundId": 3,                                   // 当前轮次ID
      "orderTypes": [
        "LIMIT",
        "MARKET"
      ],                                                     // 支持的订单类型
      "settleIntervalSeconds": 5,                            // TWAP 价格采样间隔（秒）
      "settlementDataPointCount": 5,                         // TWAP 价格数据点数量
      "reduceOnly": false,                                   // 是否只允许减仓订单
      "category": "",                                        // 事件分类
      "timeInForce": [
        "GTC",
      ...
      "liveRound": {                                         // 当前进行中的轮次
        "marketName": "USD1_UP_DOWN_5M_1778483280",          // 预测市场名称
        "symbols": [
        ...
        "roundId": 2,                                        // 轮次ID
        "tradingStartTime": 1778482970000,                   // 交易开始时间（Unix 毫秒）
        "tradingEndTime": 1778483520000,                     // 交易结束时间（Unix 毫秒）
        "windowStartTime": 1778483280000,                    // 窗口开始时间（Unix 毫秒）
        "windowEndTime": 1778483580000,                      // 窗口结束时间（Unix 毫秒）
        "roundStatus": "SETTLED",                            // 市场状态
        "openPrice": "0.99920000",                           // 开盘价
        "endPrice": "0.99910000",                            // 结算价
        "winAsset": "USD1_UP_DOWN_5M_1778483280_N",          // 获胜资产：Y 代表涨，N 代表跌
        "settleTime": 1778483582057,                         // 结算时间（Unix 毫秒）
        "settleHash": "0x3bc0aff52472dafd4b70d3423386a2dabf675fe7cd604e92391e2a4702cf0980"  // 链上结算哈希
      },
      "rounds": [                                            // 历史轮次
        {
          "marketName": "USD1_UP_DOWN_5M_1778483280",        // 预测市场名称
...
RoundStatus | Description
-----------| --------------
`PREMARKET` | 预先准备好的市场(可以提前交易)
`TRADING` | 交易中的市场
`SETTLED` | 已结算的市场

* 结算完成一段时间(24H)后，会自动下架，下架后的市场不再展示
* `status` 为`CLOSED`或者`INVALID`的事件对应的市场，只能下reduceOnly的Market Sell订单

`predictionType` 为预测事件类型：

| PredictionType | 描述 |
| :---- | :---- |
| `UP_DOWN` | 价格方向预测（涨或跌） |
| `MULTIVARIATE` | 多变量预测，有多个可能结果 |
| `MARKETONLY_MULTIPREDICT` | 仅白名单做市商可用的多结果预测 |

...
| 名称 | 类型 | 是否必需 | 描述 |
| ----------- | ---- | -------- | -----------------------------------|
| batchOrders | LIST | YES | 订单列表，最多5个订单（做市商白名单账户最多10个） |

**其中 ``batchOrders`` 应以 JSON 列表格式填写订单参数**

| 名称 | 类型 | 是否必需 | 描述 |
| ----------------- | ------- | -------- | ----------- |
| symbol | STRING | YES | 交易对 |
| orderId | STRING | NO | 订单号 |
| origClientOrderId | STRING | NO | 用户自定义订单号 |
| quantity | STRING | YES | 订单数量 |
| price | STRING | YES | 订单价格 |

* 每笔订单须传 `orderId` 或 `origClientOrderId` 之一；若同时传入，以 `orderId` 为准。
* 每笔订单的 `quantity` 和 `price` 均为必填。
* 各元素的参数规则与上方「修改订单」相同——仅支持 `LIMIT` 订单，受 `PRICE_FILTER` / `PERCENT_FILTER` / `LOT_SIZE` 限制，单笔订单最多修改 10000 次。
* 批量订单并发处理，处理顺序不保证，但响应顺序始终与请求顺序一致。
* 非原子性——批次中每笔订单独立成功或失败，修改失败的订单保持原状，不影响其他订单。
* 超出批量上限返回：`"Illegal characters found in parameter 'batchOrders'; legal range is '1 to 5'."` （做市商白名单账户为 `'1 to 10'`）。
...
| nonce | LONG | YES | 微秒级时间戳，用于防重放攻击 |
| user | STRING | NO | 签名账户的钱包地址（通常为母账户地址；子账户向母账户划转时为子账户地址）。可选——传 `user` 或 `signer` 之一即可 |
| signer | STRING | NO | 与 `user` 关联的 Agent 钱包地址，须通过 `registerAndApproveAgent`/`approveAgent` 注册授权。可选——传 `user` 或 `signer` 之一即可 |
| fromAccountAddress | STRING | NO | 转出钱包地址。当转出账户与 `user` 不一致时必传（如子→子划转，或以第三方身份发起的母→子划转） |
| signature | STRING | YES | 对消息体的签名，使用实际传入的账户对应的私钥签名：传 `user` 时使用 `user` 的钱包私钥，传 `signer` 时使用 Agent 钱包私钥（见下方签名说明） |

**`参数取值` :**

| 转账 | user | signer | fromAccountAddress | toAccountAddress | signature |
|------|------|------|------|------|------|
| 母账户转子账户 | *(可选 — 母账户地址)* | *(可选 — 母账户的 Agent)* | 母账户地址 | 子账户地址 | 使用母账户（或 Agent）私钥签名 |
| 子账户转母账户 | *(可选 — 母账户地址)* | *(可选 — 母账户的 Agent)* | 子账户地址 | 母账户地址 | 使用母账户（或 Agent）私钥签名 |
| 子账户转子账户 | *(可选 — 母账户地址)* | *(可选 — 母账户的 Agent)* | 子账户地址A | 子账户地址B | 使用母账户（或 Agent）私钥签名 |
| 子账户转母账户 | *(可选 — 子账户地址)* | *(可选 — 子账户的 Agent)* | 子账户地址 | 母账户地址 | 使用子账户（或 Agent）私钥签名 |
| 子账户转子账户 | *(可选 — 子账户A地址)* | *(可选 — 子账户A的 Agent)* | 子账户地址A | 子账户地址B | 使用子账户（或 Agent）私钥签名 |

> `user` 和 `signer` 均为可选——传其中一个即可标识签名账户（`signer` 须为经该 `user` 授权的 Agent 钱包）。签名须使用实际传入账户的私钥：传 `user` 则用 `user` 的钱包私钥，传 `signer` 则用 Agent 钱包私钥。
...
使用实际传入账户对应的私钥对以下消息体签名——传 `user` 时使用 **`user` 账户的钱包私钥**，传 `signer` 时使用 **Agent 钱包私钥**：
...
| 账户类型 | 签名算法 | 编码格式 |
|---|---|---|
| EVM 地址 | EIP-712 Typed Data（chainId=714，message.msg=消息体） | Hex |
| Solana 地址 | Ed25519 | Base58 |
...
* `user` 和 `signer` 均为可选——传其中一个即可，只需一个即可标识签名账户。
* `signature` **必须使用实际传入账户对应的私钥签名**——传 `user` 则用 `user` 的钱包私钥，传 `signer` 则用 Agent 钱包私钥。
* `signer` 必须是与 `user` 关联的 Agent 钱包地址，须通过 `POST /fapi/v3/registerAndApproveAgent` 或 `approveAgent`（具有 `canSpotTrade` 或 `canPerpTrade` 权限）注册授权。

...
* **Nonce** 用于校验请求的**有效性、唯一性和防重放**。客户端应使用**当前时间戳（微秒级）**作为 nonce，且仅在服务端时间**前后 60 秒**的窗口内有效。
...
* nonce 状态（近期 nonce 列表及上述校验）维护在 **Agent 地址级别**，而非账户/用户级别——同一账户下的每个 Agent 地址独立维护、互不影响。

* 为提升性能，每个 Agent 地址仅维护**最近 100 个 nonce**：
